...
constexpr double EncoderTicksPerInch =
    (MotorGearRatio * 2048.0) / (std::numbers::pi * 1.5);
...
constexpr units::meters_per_second_squared_t MaxAcceleration{
...
struct SwerveConfig {
  ...
  frc::Translation2d m_frontLeftLocation{WheelBaseLength / 2.0,
                                         WheelBaseWidth / 2.0};
  frc::Translation2d m_frontRightLocation{WheelBaseLength / 2.0,
                                          -WheelBaseWidth / 2.0};
  frc::Translation2d m_rearLeftLocation{-WheelBaseLength / 2.0,
                                        WheelBaseWidth / 2.0};
  frc::Translation2d m_rearRightLocation{-WheelBaseLength / 2.0,
                                         -WheelBaseWidth / 2.0};
  ...
  phoenix::motorcontrol::SupplyCurrentLimitConfiguration
      driveMotorCurrentLimits;
  ...
  // SwerveModuleConstants frontLeftConstants{1, 3, 13, DriveCANBus,
  // DriveCANBusPeriod}; SwerveModuleConstants frontRightConstants{7, 8, 12,
  // DriveCANBus, DriveCANBusPeriod};
  SwerveModuleConstants frontRightConstants{1, 3, 13, DriveCANBus,
                                            DriveCANBusPeriod};
  SwerveModuleConstants frontLeftConstants{7, 8, 12, DriveCANBus,
                                           DriveCANBusPeriod};
  SwerveModuleConstants rearLeftConstants{5, 6, 14, DriveCANBus,
                                          DriveCANBusPeriod};
  SwerveModuleConstants rearRightConstants{2, 4, 15, DriveCANBus,
                                           DriveCANBusPeriod};
  // SwerveModuleConstants rearRightConstants{2, 10, 15, DriveCANBus,
  // DriveCANBusPeriod};
  ...
  SwerveConfig() {
    ...
    driveMotorConfig.initializationStrategy =
    ...
    turnMotorConfig.initializationStrategy =
        phoenix::sensors::SensorInitializationStrategy::BootToZero;

    encoderConfig.absoluteSensorRange =
        phoenix::sensors::AbsoluteSensorRange::Signed_PlusMinus180;
    encoderConfig.sensorDirection = canCoderInvert;
    encoderConfig.initializationStrategy =
        phoenix::sensors::SensorInitializationStrategy::BootToAbsolutePosition;
