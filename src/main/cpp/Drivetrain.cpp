...
Drivetrain::Drivetrain(constants::swerveConstants::SwerveConfig constants)
    : m_frontLeft("Front Left", constants.frontLeftConstants),
      ...
      m_gyro(constants::swerveConstants::PigeonID,
             constants::swerveConstants::DriveCANBus),
      m_kinematics(constants.m_frontLeftLocation,
                   constants.m_frontRightLocation, constants.m_rearLeftLocation,
                   constants.m_rearRightLocation),
      m_odometry(m_kinematics, GetHeading(),
                 {m_frontLeft.GetPose(), m_frontRight.GetPose(),
                  m_backLeft.GetPose(), m_backRight.GetPose()}) {
  ...
  SetDriveInverted(constants::swerveConstants::DriveLeftInvert,
                   constants::swerveConstants::DriveRightInvert);
  m_gyro.SetStatusFramePeriod(
...
void Drivetrain::DrivePos(units::meter_t x, units::meter_t y,
                          units::degree_t heading) {
  frc::Pose2d pose = GetPose();
  SetTargetHeading(heading);

  units::meters_per_second_t vx(constants::swerveConstants::PosGain *
                                (x - pose.X()).value());
  units::meters_per_second_t vy(constants::swerveConstants::PosGain *
                                (y - pose.Y()).value());
  units::radians_per_second_t w(
      CalcRotationSpeed(heading, pose.Rotation().Degrees()));
  Drive(vx, vy, w);
}

units::radians_per_second_t
Drivetrain::CalcRotationSpeed(units::degree_t targetAngle,
                              units::degree_t currentHeading) {
  // frc::SmartDashboard::PutNumber("Turn Error Current Heading",
  // currentHeading.value()); find difference between goal and current heading
  ...
  frc::Rotation2d e =
      frc::AngleModulus(targetAngle - GetPose().Rotation().Degrees());
  return m_rotGain * (e.Degrees() / 180_deg) *
         constants::swerveConstants::MaxAngularVelocity;
}

void Drivetrain::DriveXY(units::meters_per_second_t xSpeed,
                         units::meters_per_second_t ySpeed) {
  frc::Rotation2d heading = GetPose().Rotation();
  units::radians_per_second_t rot =
      CalcRotationSpeed(m_targetHeading, heading.Degrees());
  auto states = m_kinematics.ToSwerveModuleStates(
...
void Drivetrain::InitSendable(wpi::SendableBuilder &builder) {
  ...
  builder.AddDoubleProperty(
      "X Position meters", [this] { return GetPose().X().value(); }, nullptr);
  builder.AddDoubleProperty(
      "X Position inches",
      [this] { return units::inch_t(GetPose().X()).value(); }, nullptr);
  builder.AddDoubleProperty(
      "Y Position meters", [this] { return GetPose().Y().value(); }, nullptr);
  builder.AddDoubleProperty(
      "Y Position inches",
      [this] { return units::inch_t(GetPose().Y()).value(); }, nullptr);
  builder.AddDoubleProperty(
      "Heading Deg", [this] { return GetPose().Rotation().Degrees().value(); },
      nullptr);
  builder.AddDoubleProperty(
      "Raw Heading Deg", [this] { return m_gyro.GetYaw(); }, nullptr);

  builder.AddDoubleProperty(
      "X Goal MPS", [this] { return m_vx_goal.value(); }, nullptr);
  builder.AddDoubleProperty(
      "Y Goal MPS", [this] { return m_vy_goal.value(); }, nullptr);
  builder.AddDoubleProperty(
      "W Goal deg/sec",
      [this] { return units::degrees_per_second_t(m_vw_goal).value(); },
      nullptr);
}
...
void Drivetrain::SendData() {
  ...
  frc::SmartDashboard::PutNumber("X Position inches",
                                 units::inch_t(GetPose().X()).value());
  // frc::SmartDashboard::PutNumber("Y Position meters", GetPose().Y().value());
  frc::SmartDashboard::PutNumber("Y Position inches",
                                 units::inch_t(GetPose().Y()).value());
  frc::SmartDashboard::PutNumber("Heading Deg",
                                 GetPose().Rotation().Degrees().value());
  ...
  frc::SmartDashboard::PutNumber(
      "W Goal deg/sec", units::degrees_per_second_t(m_vw_goal).value());
