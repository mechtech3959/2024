...
void Elevator::SetHeight(units::inch_t pos) {
  ...
  m_motor1.Set(ControlMode::MotionMagic,
               pos.value() * constants::elevatorConstants::EncoderTicksPerInch);
}

units::inch_t Elevator::GetHeight() {
  return units::inch_t(m_motor1.GetSelectedSensorPosition() /
                       constants::elevatorConstants::EncoderTicksPerInch);
  ;
}
...
void Elevator::SendData(LoggingLevel verbose) {
  ...
  {
    frc::SmartDashboard::PutNumber("Elevator Raw position",
                                   m_motor1.GetSelectedSensorPosition());
  }
    // continue
  case LoggingLevel::PID: // send PID (closed loop control) data
  {
    frc::SmartDashboard::PutNumber("Elevator Error",
                                   ctreHelpers::CTRE_Get_PID_Error(m_motor1));
    frc::SmartDashboard::PutNumber("Elevator Target",
                                   ctreHelpers::CTRE_Get_PID_Target(m_motor1));
    frc::SmartDashboard::PutNumber("Elevator PID Output%",
                                   m_motor1.GetMotorOutputPercent());
  }
    // continue
