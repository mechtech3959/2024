...
void Robot::RobotInit() {
  ...
  m_autoChooser.SetDefaultOption(a_2PieceCorridor,
                                 AutoRoutine::k2PieceCorridor);
...
bool Robot::FuseLL() {
  ...
  if ((units::math::abs(m_swerve.GetChassisSpeeds().vx) +
       units::math::abs(m_swerve.GetChassisSpeeds().vy)) > .02_mps) {
  ...
  if (m_leftLL.IsTargetVisible() && m_rightLL.IsTargetVisible()) {
    ...
    frc::Pose2d pos{
        (m_leftLL.GetRobotPose().X() + m_rightLL.GetRobotPose().X()) / 2.0,
        (m_leftLL.GetRobotPose().Y() + m_rightLL.GetRobotPose().Y()) / 2.0,
        frc::Rotation2d((m_leftLL.GetRobotPose().Rotation().Degrees() +
                         m_rightLL.GetRobotPose().Rotation().Degrees()) /
                        2.0)};
...
bool Robot::FuseLLNoHeading() {
  ...
  if (m_leftLL.IsTargetVisible()) {
    frc::SmartDashboard::PutString("Fuse LL", "Left Only");
    m_swerve.SetPose(
        {m_leftLL.GetRobotPose().X(), m_leftLL.GetRobotPose().Y(), heading});
    return true;
  }
  ...
  if (m_rightLL.IsTargetVisible()) {
    frc::SmartDashboard::PutString("Fuse LL", "Right Only");
    m_swerve.SetPose(
        {m_rightLL.GetRobotPose().X(), m_rightLL.GetRobotPose().Y(), heading});
    return true;
  }
}
...
void Robot::RobotPeriodic() {
  ...
  frc::SmartDashboard::PutNumber("Compressor Pressure (psi)",
...
void Robot::Gen2PieceCorridor() {
  ...
  */
  ...
  if (frc::DriverStation::GetAlliance() ==
    ...
    std::vector<frc::Translation2d> wp2Piece1{waypoints::BlueCorridorNear,
                                              waypoints::BlueCorridorFar};

    // create traj from score position 1 to pickup position 1
    traj2Piece1 = frc::TrajectoryGenerator::GenerateTrajectory(
        x, wp2Piece1,
        frc::Pose2d(waypoints::BluePiece1.X() - 30_in,
                    waypoints::BluePiece1.Y() + 24_in, frc::Rotation2d(0_deg)),
        c.config);
    ...
    // create waypoint list from scoring position to piece pickup position
    std::vector<frc::Translation2d> wp2Piece1{waypoints::RedCorridorNear,
                                              waypoints::RedCorridorFar};

    // create traj from score position 1 to pickup position 1
    traj2Piece1 = frc::TrajectoryGenerator::GenerateTrajectory(
        x, wp2Piece1,
        frc::Pose2d(waypoints::RedPiece1.X() + 30_in,
                    waypoints::RedPiece1.Y() + 24_in, frc::Rotation2d(0_deg)),
        c.config);
  }
}
...
void Robot::GenTraj() {
  ...
  case AutoRoutine::k2PieceCorridor:
    Gen2PieceCorridor();
    break;
  case AutoRoutine::kSimpleSwitch:
    GenSimpleSwitch();
    break;
  case AutoRoutine::kSpeedBump:
    GenSpeedBump();
    break;
