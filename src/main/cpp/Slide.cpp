...
void Slide::configDevices() {
  ...
  config.motionCruiseVelocity =
      (constants::slideConstants::EncoderTicksPerInch * inchesPerSec) / 10.0;
  config.motionAcceleration =
      5.0 *
      config
          .motionCruiseVelocity; // 1/6 sec for arm to achieve cruising velocity
...
void Slide::SetPosition(units::inch_t position) {
  ...
  m_target = position;
  m_motor.Set(ControlMode::MotionMagic,
              constants::slideConstants::EncoderTicksPerInch *
                  position.value());
}
...
units::inch_t Slide::GetPosition() {
  return units::inch_t(m_motor.GetSelectedSensorPosition() /
                       constants::slideConstants::EncoderTicksPerInch);
  // returns position
}
...
void Slide::SendData(LoggingLevel verbose) {
  ...
  {
    frc::SmartDashboard::PutNumber("raw Slide position",
                                   m_motor.GetSelectedSensorPosition());
  }
    // continue
  case LoggingLevel::PID: // send PID (closed loop control) data
  {
    frc::SmartDashboard::PutNumber("Slide Error",
                                   ctreHelpers::CTRE_Get_PID_Error(m_motor));
    frc::SmartDashboard::PutNumber("Slide Target",
                                   ctreHelpers::CTRE_Get_PID_Target(m_motor));
    frc::SmartDashboard::PutNumber("Slide PID Output%",
                                   m_motor.GetMotorOutputPercent());
  }
    // continue
  case LoggingLevel::Basic: // minimal useful data to driver
  {
    frc::SmartDashboard::PutNumber("Slide position (in)",
                                   GetPosition().value());
  }
    // continue
  default:
